<template>
  <el-dialog :title="isEdit ? '编辑' : '新增'" :visible.sync="visible" width="600px" :before-close="close"
      ...
      <el-form label-width="150px" :model="unitManageForm" ref="unitManageForm" :rules="rules" inline>
        <ss-common-area-group :columnCount="1" style="width：100%">
          ...
          <ss-common-area-item label="接种单位名称:" prop="schStationName" v-if="isEdit">
            <el-input v-model="unitManageForm.schStationName" disabled style="width: 250px" placeholder="请选择接种单位名称"></el-input>
          </ss-common-area-item>
          ...
          <ss-common-area-item label="学校编码:" prop="schCode">
            <el-input clearable v-model.trim="unitManageForm.schCode" maxlength="3" :disabled="isEdit ? true : false"
              style="width: 250px" placeholder="请输入学校编码"></el-input>
          </ss-common-area-item>
          <ss-common-area-item label="学校名称:" prop="schName">
            <el-input clearable style="width: 250px" maxlength="18" v-model.trim="unitManageForm.schName" placeholder="请输入学校名称"></el-input>
          </ss-common-area-item>
          <ss-common-area-item label="学校类型:" prop="schType">
            <el-select v-model="unitManageForm.schType" clearable placeholder="请选择学校类型">
              <el-option v-for="item in $store.state.selectTypeList" :key="item.code" :label="item.name" :value="item.code" />
            </el-select>
          </ss-common-area-item>
          <ss-common-area-item label="地址:" prop="schAddress">
            <el-input clearable style="width: 250px" maxlength="26" v-model.trim="unitManageForm.schAddress" placeholder="请输入地址"></el-input>
          </ss-common-area-item>
          <ss-common-area-item label="电话:" prop="schPhone">
            <el-input clearable style="width: 250px" maxlength="26" v-model.trim="unitManageForm.schPhone" placeholder="请输入电话"></el-input>
          </ss-common-area-item>
          <ss-common-area-item label="联系人:" prop="schLinkMan">
            <el-input clearable style="width: 250px" maxlength="26" v-model.trim="unitManageForm.schLinkMan" placeholder="请输入联系人"></el-input>
          </ss-common-area-item>
...
<script>
import { insertSchool, schoolDetail, updateSchool } from '@/utils/axios/apis/showSchoolManageQuery'
      ...
      if (v) {
        if (this.isEdit) {
          schoolDetail({ schId: this.curId }).then((res) => {
            ...
            this.unitManageForm.schPhone = res.data.schPhone
  ...
  methods: {
    ...
    save() {
      if (!this.isEdit) {
        ...
        this.unitManageForm.areaId = this.areaTreeValue.schStationId
        this.unitManageForm.areaCode = this.areaTreeValue.schStationCode
        this.unitManageForm.areaName = this.areaTreeValue.schStationName
        this.unitManageForm.supCode = this.areaTreeValue.supCode
      }
      const data = {
        ...this.unitManageForm
      }
