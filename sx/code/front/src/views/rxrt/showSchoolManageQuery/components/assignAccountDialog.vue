...
<template>
  <el-dialog title="账号分配" :visible.sync="visible" width="40%" :before-close="close" :close-on-click-modal="false"
    ...
    <div class="condition-row mt_20">
      <div v-if="msgPubPermission">
        <el-button size="small" type="primary" @click="addAccount">新增账号</el-button>
        <div class="ml20">备注：点击【新增账号】 或 【重置密码】，对应账号的密码默认为{{schTypeIsNew?'SXcyxt@123456':'NBcyxt@123456'}}</div>
      </div>
      <div v-else>账号的密码默认为{{schTypeIsNew?'SXcyxt@123456':'NBcyxt@123456'}}</div>
    </div>
    <ss-table :data="dataList" ref="tabPage" class="assignTable" :maxHeight="200">
      ...
      <el-table-column align="center" min-width="150" label="操作" v-if="schTypeIsNew && msgPubPermission">
...
<script>
import { schUserList, addSchUser, resetSchUser } from '@/utils/axios/apis/showSchoolManageQuery'
export default {
  ...
  watch: {
    visible (v) {
      if (v) {
        this.getUserList()
        if (this.schType == '0' || this.schType == '1') { this.schTypeIsNew = false } else { this.schTypeIsNew = true }
      }
    }
  },
  methods: {
    ...
    resetPwd (row) {
      if (row.id) {
        resetSchUser({ id: row.id, schType: this.schType }).then((res) => {
          if (res) {
            this.$message({
              type: 'success',
