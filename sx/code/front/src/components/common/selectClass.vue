  ...
  data () {
    return {
      ...
      rules: {
        ...
        schoolId: [
          { required: !!this.otherField.schoolnameRequired, message: '请选择学校名称', trigger: 'change' }
        ],
        stuclayear: [
        ...
        classId: [
          { required: !!this.otherField.classRequired, message: '请输入班级', trigger: 'blur' }
        ]
      },
  ...
  methods: {
    ...
    findSchool () {
      let params={
        ...
        schType: this.unitManageForm.checkType==''?'0,1':this.unitManageForm.checkType
      }
      findSchoolByAreaAndSchType(params).then((res) => {
