 ...
 */
@Mapper
public interface SaasInfoMapper1 {
...
//            "when FMoveKind='6' then '国际流动' end as regtype " +
    ...
    @Select("select FChildNo fchildno, FIDCard cid, FProofNo imuno, FChildName name, FBirthday birth, FSex sex, " +
            "FMobile mobphone,FTelePhone otherphone, FFatherName fathername, FMotherName mothername, FMoveKind regtype, UStationCode stationcode,FResideAddr fResideAddr ,fOtherGuard otherLinkName ,fOtherGuardMobile otherLinkPhone " +
            "from chchild where FChildNo=#{fchildno} limit 1")
    InfoBean queryInfoByChildno(@Param("fchildno") String fchildno);
    ...
    @Select("select FChildNo fchildno, FIDCard cid, FProofNo imuno, FChildName name, FBirthday birth, FSex sex, " +
            "FMobile mobphone,FTelePhone otherphone, FFatherName fathername, FMotherName mothername, FMoveKind regtype, UStationCode stationcode,FResideAddr fResideAddr ,fOtherGuard otherLinkName ,fOtherGuardMobile otherLinkPhone " +
            "from chchild where FIDCard=#{cid} and FIDCardType='01' limit 1")
    InfoBean queryOneInfoByCid(@Param("cid") String cid);

    @DS("saas")
    @Select("select FChildNo fchildno, FIDCard cid, FProofNo imuno, FChildName name, FBirthday birth, FSex sex, " +
            "FMobile mobphone,FTelePhone otherphone, FFatherName fathername, FMotherName mothername, FMoveKind regtype, UStationCode stationcode,FResideAddr fResideAddr ,fOtherGuard otherLinkName ,fOtherGuardMobile otherLinkPhone " +
            "from chchild where FIDCard=#{cid} and FIDCardType='01'")
     ...
     */
    @DS("saas")
    @Select("select FChildNo stu_no, FIDCard cid, FProofNo stu_imuno, FChildName stu_name, FBirthday stu_birth, FSex stu_sex, " +
            "FMobile stu_phone, FMobile stuMotherPhone,  FTelePhone stuFatherPhone, FFatherName stuFathername, FMotherName stuMothername, FMoveKind stu_type, concat(UStationCode,'00') stuBelongStationcode ,FResideAddr fResideAddr,fOtherGuard otherLinkName ,fOtherGuardMobile otherLinkPhone " +
            "from chchild where FChildNo=#{fchildno} limit 1")
    ...
    @Select("SELECT s.FBactID fbactid, date_format(FShotDate,'%Y-%m-%d') jzrq, FShotKind fShotKind, FBactType fBactType, CONCAT(FBactShortName, FBactSeq) fBactName " +
            ...
            "INNER JOIN vaccin_info v ON v.FBactID = s.FBactID " +
            "WHERE length(FShotDate) > 0 AND length(FShotKind) > 0 " +
            "AND FChildNo=(SELECT FChildNo FROM chchild WHERE FIDCard=#{cid} AND FIDCardType='01' limit 1) " +
            "ORDER BY v.FBactType, FShotDate ASC")
