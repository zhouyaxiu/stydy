...
public class RoundServiceImpl implements RoundService {
    ...
    @Override
    public PageInfo<Round> findRounds(String areaCode ,String areaLevel,String nowYear, String vaccName, String roundName,String state,int pageNum ,int pageSize) {
        //处理关联表
       ...
       if("1".equals(state)){
        ...
        List<Round> list  = roundMapper.findRounds(areaCode,areaLevel,nowYear,vaccName,roundName,build.toString(),state);
        //处理每个轮次对应的学校信息
        list.forEach(round -> {
            String  schIds  = round.getSchIds();
            String clsIds  =round.getClaIds();
            round.setRoundName("第"+round.getRoundName()+"轮");
            if(StringUtils.isNotEmpty(schIds)) {
                String[] schId = schIds.split(",");
