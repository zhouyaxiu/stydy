...
public class RevaccinationStatUtil {
     ...
     * 未全程接种不需补种
     */
    public static String NOT_VACCINE_ALL_NOT = "-1";
    /**
     * 已种
     */
        ...
        for (CheckPassJzjl checkPassJzjl : list) {
            ...
            try {
                if (StringUtils.isNotBlank(fbactid) && StringUtils.isNotBlank(shotDate) && pattern.matcher(fbactid.toLowerCase()).find()) {
                    ...
                    //初次查验已种、禁忌、已患，则不需要再做补种
                    /*if (hasVaccineState.equals(lastState) || notVaccineButNotNeedRestockState.equals(lastState)) {
                        continue;
                    }*/
                    ...
                    if (DateUtil.isDate(shotDate)) { //合法的日期，不包含特殊日期
                        ...
                        if (SPECIAL_SHOT_DATE_MAP.containsKey(shotDate)) { //特殊日期状态
                            writeMethod.invoke(stuinfo, SPECIAL_SHOT_DATE_MAP.get(shotDate).getState()); //1，4，6，7算已种，3，5算未种需提示补种，2，8算未种但是不需提示补种。
                        }
                    }
