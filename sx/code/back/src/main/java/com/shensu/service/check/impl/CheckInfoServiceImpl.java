...
public class CheckInfoServiceImpl implements CheckInfoService {
    ...
    @Override
    public PageInfo<StudentBase> getList(Integer level, String areaCode, String schType, String schId, String year, String rounds, String grade, String clazz,
        ...
        List<StudentBase> studentList = checkInfoMapper.queryStudentList(level, areaCode, schType, schId, year, rounds, grade, clazz, isjz, sqlState, name, idcard, code);
    ...
    @Override
    public PageInfo<Student> getReplantRosterPage(Integer level, String areaCode, String schType, String schId, String year, String rounds, String grade, String className ,Integer pageNum, Integer pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        List<Student> studentList = checkInfoMapper.queryReplantRosterList(level, areaCode, schType, schId, year, rounds, grade,className);
    ...
    @Override
    public PageInfo<JSONObject> getReplantVaccinumStatistics(Integer level, String areaCode, String schType, String year, String rounds, String birthStart, String birthEnd, String replantStart, String replantEnd, String isFull,String schId,String grade,String className) {
        ReplantVacStatBean statBean = getReplantVaccinumStat(level, areaCode, schType, year, rounds, birthStart, birthEnd, replantStart, replantEnd, isFull,schId,grade,className);
        ...
        list.add(json);
        // 百白破
        json = initReplantVacStatValue("百白破疫苗", statBean.getBaiBaiPoNeedReplantSubtotal(), statBean.getBaiBaiPoHasReplantSubtotal(), statBean.getBaiBaiPoHasReplantRate());
        list.add(json);
        // 白破
        ...
        list.add(json);
        // A群流脑疫苗
        json = initReplantVacStatValue("A群流脑疫苗", statBean.getAQunLiuNaoNeedReplantSubtotal(), statBean.getAQunLiuNaoHasReplantSubtotal(), statBean.getAQunLiuNaoHasReplantRate());
        list.add(json);
        // A+C群流脑疫苗
        json = initReplantVacStatValue("A+C群流脑疫苗", statBean.getACQunLiuNaoNeedReplantSubtotal(), statBean.getACQunLiuNaoHasReplantSubtotal(), statBean.getACQunLiuNaoHasReplantRate());
        list.add(json);
        // 乙脑
        ...
        list.add(json);
        // 甲肝
        json = initReplantVacStatValue("甲肝灭活疫苗", statBean.getJiaGanJianDuHuoNeedReplantSubtotal(), statBean.getJiaGanJianDuHuoHasReplantSubtotal(), statBean.getJiaGanJianDuHuoHasReplantRate());
        list.add(json);
        // 合计
    ...
    @Override
    public ReplantVacStatBean getReplantVaccinumStat(Integer level, String areaCode, String schType, String year, String rounds, String birthStart, String birthEnd, String replantStart, String replantEnd, String isFull,String schId,String grade,String className) {
        ReplantVacStatBean statBean = checkInfoMapper.queryReplantVaccinumStat(level, areaCode, schType, year, rounds, birthStart, birthEnd, replantStart, replantEnd, isFull,schId,grade,className);
        //合并
        //卡介苗
    ...
    public PageInfo<AreaCdcUserVacData> getVacCertCheckStat(Integer level, String areaCode, String schType, String year, String rounds) {
       // List<AreaCodeAndName> listArea =  sysAreaMapper.findCodeAndName(areaCode,level+"");
      ...
      /*  for(int i=0;i<dataList.size();i++){
            AreaCdcUserVacData nn = dataList.get(i);
            for(int j=0;j<listArea.size();j++){
                AreaCodeAndName  areaMess  = listArea.get(j);
                if(nn.getAreaCode().equals(areaMess.getAreaCode())){
                    nn.setAreaName(areaMess.getAreaName());
                }
            }
        }*/
        //计算合计
    ...
    public void outPut(ExcelUtil excelUtil, JSONObject info, List<StudentBase> list) {
        ...
        String supArea = "选择地区：" + areaName + " " + areaCode;
        StringBuilder  bb  = new StringBuilder();
        bb.append(supArea);
        if (StringUtils.isNotBlank(schType)) {
            bb.append("       学校类型：" + Constant.SCHOOL_TYPE_MAP.get(schType));
        }
        if (StringUtils.isNotBlank(schName)) {
            bb.append("       学校名称：" + schName);
        }
        ...
        for (StudentBase bean : list) {
            ...
            excelUtil.fillCellData(rownum, (short) colnum++, DateUtil.formatShort(bean.getStuUpdatetime()), style);     //查验日期
    ...
    public void outPutReplantRoster(ExcelUtil excelUtil, JSONObject info, List<Student> list) {
        ...
        buffer.append("选择地区：" + areaName);
        if (StringUtils.isNotBlank(schType)) {
            buffer.append(" 学校类型：" + Constant.SCHOOL_TYPE_MAP.get(schType));
        }
        if (StringUtils.isNotBlank(schName)) {
            buffer.append(" 学校名称：" + schName);
        }
        if (StringUtils.isNotBlank(year)) {
    ...
    @Override
    public void outputReplantVacStat(ExcelUtil excelUtil, JSONObject info, ReplantVacStatBean statBean) {
        //设置查询条件
        String areaName = info.getString("areaName");
        String areaCode = info.getString("areaCode");
        String schType = info.getString("schType");
        ...
        int titleNum = 2;
        if (StringUtils.isNotBlank(schType)) {
            excelUtil.fillCellData(1, (short) titleNum, "学校类型：" + Constant.SCHOOL_TYPE_MAP.get(schType));
            titleNum += 2;
        }
        if (StringUtils.isNotBlank(schName)) {
        ...
        if (statBean != null) {
            ...
            excelUtil.fillCellData(rownum, (short) 1, statBean.getKaJieNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getKaJieHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getKaJieHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "乙肝疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getYiGanNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getYiGanHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getYiGanHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "脊灰疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getJiHuiNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getJiHuiHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getJiHuiHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "百白破疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getBaiBaiPoNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getBaiBaiPoHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getBaiBaiPoHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "白破疫苗疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getBaiPoNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getBaiPoHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getBaiPoHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "麻腮风疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getMaFengNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getMaFengHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getMaFengHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "A群流脑疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getAQunLiuNaoNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getAQunLiuNaoHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getAQunLiuNaoHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "A+C群流脑疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getACQunLiuNaoNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getACQunLiuNaoHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getACQunLiuNaoHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "乙脑疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getYiNaoJianDuHuoNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getYiNaoJianDuHuoHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getYiNaoJianDuHuoHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "甲肝灭活疫苗", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getJiaGanJianDuHuoNeedReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getJiaGanJianDuHuoHasReplantSubtotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getJiaGanJianDuHuoHasReplantRate(), style);

            rownum++;
            excelUtil.fillCellData(rownum, (short) 0, "合计", style);
            excelUtil.fillCellData(rownum, (short) 1, statBean.getNeedReplantTotal(), style);
            excelUtil.fillCellData(rownum, (short) 2, statBean.getHasReplantTotal(), style);
            excelUtil.fillCellData(rownum, (short) 3, statBean.getHasReplantTotalRate(), style);
        }
    ...
    public void outputVacCertCheckStat(ExcelUtil excelUtil, JSONObject info, List<AreaCdcUserVacData> list) {
        ...
        String year = info.getString("year");
        String rounds = info.getString("rounds");
        String level = info.getInteger("areaLevel")+"";
        String supArea = "填报单位：" + areaName + " " + areaCode;
        if(!"4".equals(level)){
            excelUtil.fillCellData(1, (short) 0, "（省、市、县、乡级使用，逐级汇总上报）");
        }
        if (com.shensu.util.StringUtils.isNotEmpty(areaName)) {
            excelUtil.fillCellData(2, (short) 0, "填报单位：" + areaName);
        }

        if (com.shensu.util.StringUtils.isNotEmpty(rounds)) {

            excelUtil.fillCellData(2, (short) 4, "轮次：" + ("0".equals(rounds)?"春季（  ） 秋季（√ ）":" 春季（ √ ） 秋季（ ）"));
        }

        if (com.shensu.util.StringUtils.isNotEmpty(schType)) {
            excelUtil.fillCellData(2, (short) 8, "入学类型：" + ("0".equals(schType) ? "入托（ √ ） 入学（ ）":("1".equals(schType)?"入托（ ） 入学（ √ ）":"入托（ ） 入学（ ）")));
        }

        excelUtil.fillCellData(2, (short) 12, "填报日期：" + DateUtils.getDate("yyyy-MM-dd"));

        int totalCount = list.size();// 记录总条数
        int firstRowNum =8;// 记录在excel文件中的起始行号
        HSSFFont font = excelUtil.createFont();
        font.setFontHeightInPoints((short) 9);
        font.setFontName("宋体");
        HSSFCellStyle style = excelUtil.createCellStyle();
        ...
        style.setFont(font);// 设置样式的字体
        style.setWrapText(true);//自动换行
        Iterator<AreaCdcUserVacData> iterator = list.iterator();
        int curRow = 0;
        //构建所有的行和列
        for (int j = 0; j < list.size() ; j++) {
            curRow = firstRowNum + j ;
        ...
        for (AreaCdcUserVacData bean : list) {
            ...
            colnum = 0;
            //excelUtil.fillCellData(rownum, (short) colnum++, String.valueOf(i+1), style);     //序号
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getAreaName());     //报告单位
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getNeedCySchNum());     //托幼机构小学应查验数
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getNeedCySchNum());     //托幼机构小学查验数
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getStuCyNum());     //入托入学人数(所有儿童数)
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getStuCyNum());     //查验人数(已查验的儿童数)
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getHasCardNum());     //持预防接种证人数
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getPatchCardNum());     //补预防接种证人数
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getQcjzCount());     //完成全程接种人数
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getNeedReplantNum());     //需补种疫苗人数
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getHasReplant(), style);     //完成全程补种人数

            excelUtil.fillCellData(rownum, (short) colnum++, bean.getKajieNeed());     //卡介苗应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getKajieHas());     //卡介苗补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getYiganNeed());     //乙肝应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getYiganHas());     //乙肝补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getJihuiNeed());     //脊灰应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getJihuiHas());     //脊灰补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getBaibaipoNeed());     //百白破应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getBaibaipoHas());     //百白破补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getBaipoNeed());     //白破应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getBaipoHas());     //白破补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getMafengNeed());     //麻腮风应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getMafengHas());     //麻腮风补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getAqunliunaoNeed());     //A群流脑应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getAqunliunaoHas());     //A群流脑补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getAcqunliunaoNeed());     //A群C群流脑应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getAcqunliunaoHas());     //A群C群流脑补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getYinaojianduNeed());     //乙脑应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getYinaojianduHas());     //乙脑补种剂次
            excelUtil.fillCellData(rownum, (short) colnum++, bean.getJiaganjianduNeed());     //甲肝应补种剂次
            excelUtil.fillCellData(rownum, (short) colnum, bean.getJiaganjianduHas());     //甲肝补种剂次
            i++;
        }
        ...
        HSSFRow mess = excelUtil.createRow(curRow);
        for (int k = 0; k < 30; k++) {
            excelUtil.createCell(mess, (short) k);
            excelUtil.setCellStyle(curRow, (short) k, style);
        }
        //处理需要展示的内容
    ...
    @Override
    public PageInfo<InforReplantMess> queryReplateMess(Integer level, String areaCode, String schType, String schId, String year, String rounds, String grade, String clazz, String isjz, String isjzz, String status, String name, String idcard, String code, List<String> rowkeyList) {
        List<Student> studentList = checkInfoMapper.queryReplateMess(level, areaCode, schType, schId, year, rounds, grade, clazz, "1", isjzz, status, name, idcard, code, rowkeyList);
        ...
        for (Student student: studentList) {
            ...
            SysStation belongStation =  taskService.findStationMess(student.getStuBelongStationcode());
    ...
    @Override
    public PageInfo<InforReplantMess> queryReplantCard(Integer level, String areaCode, String schType, String schId, String year, String rounds, String grade, String clazz, String isjz, String isjzz, String status, String name, String idcard, String code, List<String> rowkeyList) {
        List<Student> studentList = checkInfoMapper.queryReplateMess(level, areaCode, schType, schId, year, rounds, grade, clazz, isjz, "1", status, name, idcard, code, rowkeyList);
        List<InforReplantMess> list = new ArrayList<>();
        for (Student student: studentList) {
            InforReplantMess inforReplantMess = new InforReplantMess();
            inforReplantMess.setSchName(student.getSchName());
    ...
    @Override
    public PageInfo<InforReplantMess> queryNoArchive(Integer level, String areaCode, String schType, String schId, String year, String rounds, String grade, String clazz, String isjz, String isjzz, String status, String name, String idcard, String code, List<String> rowkeyList) {
        List<Student> studentList = checkInfoMapper.queryReplateMess(level, areaCode, schType, schId, year, rounds, grade, clazz, isjz, isjzz, "2", name, idcard, code, rowkeyList);
        List<InforReplantMess> list = new ArrayList<>();
        for (Student student: studentList) {
            InforReplantMess inforReplantMess = new InforReplantMess();
            inforReplantMess.setSchName(student.getSchName());
    ...
    @Override
    public PageInfo<InforReplantMess> queryCheckCert(Integer level, String areaCode, String schType, String schId, String year, String rounds, String grade, String clazz, String isjz, String isjzz, String status, String name, String idcard, String code, List<String> rowkeyList) {
        List<Student> studentList = checkInfoMapper.queryReplateMess(level, areaCode, schType, schId, year, rounds, grade, clazz, "0", isjzz, status, name, idcard, code, rowkeyList);
        List<InforReplantMess> list = new ArrayList<>();
        for (Student student: studentList) {
            InforReplantMess inforReplantMess = new InforReplantMess();
            inforReplantMess.setSchName(student.getSchName());
            ...
            //父母姓名,联系方式
            inforReplantMess.setParentName(student.getStuParentname());
            inforReplantMess.setStuFathername(student.getStuFathername());
            inforReplantMess.setStuFatherPhone(student.getStuFatherPhone());
            inforReplantMess.setStuMothername(student.getStuMothername());
            inforReplantMess.setStuMotherPhone(student.getStuMotherPhone());
            ...
            inforReplantMess.setBelongSchoolStation(student.getBelongSchoolStation());
            List<VaccinInfo> replantList = this.getReplantList(student);
            inforReplantMess.setReplantList(replantList);
            list.add(inforReplantMess);
        }
        return new PageInfo<>(list);
    }
    ...
    public List<VaccinInfo> getReplantList(Student stu) {
        //Map<String, String> map = new HashMap<>();
        List<VaccinInfo> list = new ArrayList<>();
        VaccinInfo vaccinInfo = null;
        for (int i = 0; i < Constant.fullList.size(); i++) {
            try {
                FullExcelField fef = Constant.fullList.get(i);
                Method getValueAfter = Student.class.getMethod("get" + fef.getFiledName() + "After");
                ...
                if (RevaccinationStatUtil.reachTimeButNotVaccineState.equals(invoke)) {
                    ...
                    Method getCheckValue = Student.class.getMethod("get" + Constant.VACCIN_MAP.get(fef.getFiledName()));
                    String checkVal = (String) getCheckValue.invoke(stu);
                    String ym = Constant.ymStr.get(fef.getFiledName());


                    //将疫苗名称和剂次名称分来放在map集合中
                    if (com.shensu.util.StringUtils.isNumeric(ym.substring(ym.length()-1))) {
                        vaccinInfo = new VaccinInfo(ym.substring(0,ym.length()-1), "第"+ym.substring(ym.length()-1)+"剂次", checkVal);
                    } else {
                        vaccinInfo = new VaccinInfo(ym, "第1剂次", checkVal);
                    }
                    list.add(vaccinInfo);
                }
            } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
     ...
     */
    @Transactional
    protected Integer checkChildInfo(List<Student> childList) {
