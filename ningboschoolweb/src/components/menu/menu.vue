<template>
  <el-menu :default-active="defaultActive" defaultOpenMenu="[menu0]" mode="horizontal" class="menu-plan">
    <template v-for="(itemTop, indexTop) in menus">
      <el-submenu :key="'menu' + indexTop" :index="indexTop + ''" v-if="itemTop.supModules.length" :popper-append-to-body="false">
        <template slot="title">
        ...
        <template v-for="(itemSub, indexSub) in itemTop.supModules">
          ...
          <el-submenu :index="indexTop + '-' + indexSub" :key="'menu' + indexTop + '-' + indexSub"
            v-if="itemSub.supModules && itemSub.supModules.length > 0" :popper-append-to-body="false">
