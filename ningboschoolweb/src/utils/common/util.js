...
export default {
   ...
   * @param {Boolen} isEncrypt 是否脱敏，默认为true
   * @returns {String} 手机号
  */
  encryptPhone (word, isEncrypt = true) {
    let result = this.decodeAes(word)
    if (isEncrypt) {
      return result.replace(/^(\d{3})\d+(\d{4})$/, '$1****$2')
    }
    return result
  },
  /**
   ...
   * @returns {String} 身份证号
  */
  encryptCid (word, isEncrypt = true) {
    let result = this.decodeAes(word)
    if (isEncrypt) {
      return result.replace(/(\w)/g, function (a, b, c, d) { return (c > 1 && c < (result.length - 2)) ? '*' : a })
    }
    return result
  }
}
