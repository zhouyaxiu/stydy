  ...
  <div>
    <ss-condition-area ref="conditionPage" @setConditionFlag="setConditionFlag" :titleLong="true">
      ...
      <div class="button-set">
        ...
        <!--  Excel批量导入-->
        <ss-custom-button @click="importBtn" :type="$consts.CUSTOMBUTTONTYPES.batchImport" class="mr10" />
        <batchImportBtn :visible="importVisible" @close="importClose" @upload="uploadFile" class="mr10" @progress="progress" @uploadEnd="uploadEnd" ref="uploadPro">
        </batchImportBtn>
        <downloadFile :visible="isDownFileVisible" @close="downloadFileClose"></downloadFile>
        <!-- 导入失败查询 -->
        <ss-custom-button @click="errorSearch" :type="$consts.CUSTOMBUTTONTYPES.failedQuery" />
      </div>
    </ss-condition-area>
    <div class="main-content">
      <div class="remark mb10">备注：导入者可查看对应的明文身份证件号。</div>
      <ss-table :data="dataList" ref="tabPage" class="infoTable" @goPage="find" :pagenum="pagenum" :total="total">
        ...
        <template slot-scope="{ row }">
          <span>{{ row.cid ? $util.encryptCid(row.cid,(!row.uploadUserId||row.uploadUserId!==userId)) : '' }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    uploadFile (file, type) {
      ...
      let params = {
        ...
        file: file.file,
        type: that.$refs.uploadPro.radioGroup
      }
