  ...
  <el-dialog title="选择下载内容" :visible.sync="visible" width="40%" :before-close="close" :close-on-click-modal="false" class="down">
    <div class="content">
      <el-checkbox-group v-model="checkboxGroup">
        <el-checkbox label="编码导入模板" border></el-checkbox>
...
<script>
import { downLoadImportMould } from '@/utils/axios/apis/batchImport/index'
const options = {
  '编码导入模板': '0',
  '身份证件号码导入模板': '1',
    ...
    downFile (index) {
      setTimeout(async () => {
        if (index < this.checkboxGroup.length) {
          ...
          await downLoadImportMould({ type: options[this.checkboxGroup[index]] })
          this.downFile(++index)
        } else {
          this.close()
        }
