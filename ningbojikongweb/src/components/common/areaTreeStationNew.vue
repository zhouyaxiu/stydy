 ...
 */
  ...
  <div class="china-tree">
    <div style="display: flex">
      <el-input
        v-model.trim="areaInput"
        class="areaInput"
        :readonly="true"
        placeholder=""
        @click.native="showTreeFlag = true"
        v-clickoutsideList="closeAllTreePlan"
        :disabled="disabled"
      />
      <el-select
        v-model="selectType"
    ...
    <div
      v-show="!disabled && showTreeFlag"
      :style="'position: absolute;z-index: 2006;'"
    >
      <el-tree
        class="tree-area"
        id="addressTree"
        :style="
          'width:' + treeW + ';min-width:300px;overflow-y:scroll;max-height:500px;height:' + treeHTemp
        "
        ref="chinaTree"
      ...
      >
        <span class="custom-tree-node" slot-scope="{ node, data }">
          <div v-if="selectAreaCode == data.areacode">
            ...
            {{ data.areaname }}{{ data.newBaseCode }}
          </div>
          <div v-else>
            <!-- {{ node.label }}&nbsp;{{ data.areacode }}&nbsp;&nbsp;&nbsp;&nbsp; -->
            {{ data.areaname }}{{ data.newBaseCode }}
...
import { queryAreaNew } from '@/utils/axios/api'
...
export default {
  ...
  data () {
    return {
      ...
      treeW: 'auto',
      // treeH: '500px',
      treeHTemp:this.treeH,
      scale: window.scale,
      showTreeFlag: false,
      showCheckbox: false,
      defaultExpand: [2, 185],
      selectAreaId: '',
      selectAreaCode: this.$route.query.supOrganCode+'00' || '',
      selectAreaName: '海南',
      // newBaseCode是10位地区编码
      newBaseCode: '',
      // 上级地区编码
  ...
  methods: {
    ...
    findNodeData (data, checked) {
      ...
      if(data.areacode==undefined){
        data.areacode='460000000000'
      }
      // data.areacode?data.areacode: '4600000000'
    ...
    loadNode (node, resolve) {
      ...
      if (level === 0) {
        queryAreaNew({
          areaCode: that.$route.query.supOrganCode+'00' || '460000000000'
        }).then((res) => {
          let arr = []
          if (res.length === 0) that.treeHTemp = '0px'
          if (res && res.length > 0) {
            let newBaseCode = that.$route.query.supOrganCode+'00' || '460000000000'
