import { getOptions, regTypeQuery2, regTypeTransform2, findYiMiaodEnterp, queryCrowdKind, findYiMiaoJiCi, queryAllSchoolByAreaCode, findClaYearBySchId, findClaLevelBySchId, findClassBySchId, findClassBySchIdAndSeq, findClaLevelBySchIdAndSeq,findSchoolByAreaAndSchType,findClassMess } from '@/utils/axios/api'
export default {
  data() {
    return {
      // 户籍类型
  ...
  methods: {
    ...
    async findClaLevelNew(params) {
      // await findClaLevelBySchIdAndSeq(params).then((res) => {
      //   this.gradeList = res
      // })
      if(params=='0'){
    ...
    async findClassNew(params) {
      // findClassBySchIdAndSeq(params).then((res) => {
      //   this.classesList = res
      // })
      if(params!=undefined){
