<script>
  ...
  export default {
    ...
    render(h) {
      ...
      const { expandTrigger, checkStrictly } = config;// const { expandTrigger, checkStrictly, multiple } = config;
      ...
      } else if (!isDisabled) {// } else if (!isDisabled && !checkStrictly && !multiple) {
        events.on.click = this.handleCheckChange;
      }
