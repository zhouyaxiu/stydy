...
let arrinflows=[]
const draw = async () => {
  let vaccines = [...data.value.vaccines,...data.value.vaccines]
  let outflows = [...data.value.outflows,...data.value.outflows]
  let inflows = [...data.value.inflows,...data.value.inflows]
  let option = {
    tooltip: {
      show: true,
      // appendToBody: true,
      ...
      formatter: (params) => {
        let str = `
              <div style="font-size:13px;padding:6px;">
              	<span style='color:#FFFB00'>${data.value.vaccines[params.dataIndex]}</span>
              	</br>
                <span style='color:#4ee2e5'>
                ${params.seriesName}
                </span>
