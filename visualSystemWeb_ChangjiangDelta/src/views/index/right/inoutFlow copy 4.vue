...
const draw = async () => {
  ...
  let inoutFlow = proxy.$echarts.init(document.getElementById('inoutFlow'), null, { width: 'auto', height: 364 });
  let vaccLen=data.value.vaccines.length
  let vaccines = data.value.vaccines.concat(data.value.vaccines)		
  let outflows = data.value.outflows.concat(data.value.outflows)	
  let inflows = data.value.inflows.concat(data.value.inflows)	
  timer=setInterval(()=>{
  if(option.dataZoom.endValue==vaccLen){
    console.log(option.dataZoom,'vaccines')
    data.value.vaccines=vaccines
    data.value.outflows=outflows
    data.value.inflows=inflows
    option.dataZoom.endValue=18
