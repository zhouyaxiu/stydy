<!-- 国家督察-新冠病毒疫苗分年龄、分企业接种信息汇总表 -->
<template>
  <div class="ageandfenter">
    <ss-condition-area>
      <div class="condition-row">
        ...
        <div class="item">
          ...
          <!-- <el-select v-model="sourceId" style="position: relative;" clearable>
...
<script setup>
import { ref, } from 'vue';
import { query, output } from "@/api/apis/gjdc";
...
const openDetailDialog = (row, prop) => {
  detailData.value = {
    ...searchData.value,
    areaCode: searchData.value.areaCode,
    areaLevel: searchData.value.areaLevel,
    startDate: searchData.value.startDate,
    endDate: searchData.value.endDate,
    fenterNo: searchData.value.fenterNo,
    sourceFrom: searchData.value.sourceFrom,
    age: row.age == '合计' ? '-1' : Math.max(...(row.age.match(/\d+/g))),
    jici: prop.slice(2)
  }
...
<style lang='scss'>
.ageandfenter .el-table__inner-wrapper::before {
  bottom: 1px !important;
}

.ageandfenter .el-table--enable-row-transition .el-table__inner-wrapper::before {
