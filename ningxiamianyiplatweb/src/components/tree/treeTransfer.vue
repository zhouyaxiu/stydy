    ...
    <el-row :gutter="20">
        ...
        <el-col :span="10">
            <el-card class="tree-transfer-card">
                <template #header>
                    <div class="card-header">
                        <el-checkbox v-model="rightChecked" :label="rightCardName" size="large"
                            @change="checkAll('right')" />
                    </div>
                </template>
                <el-scrollbar height="300px">
...
// 监听传过来的数据初始化穿梭框
// watch([() => {props.treeData},() => {props.defValue}],() => {
//   initCheckedKeys()
// },{deep: true})
...
const clickHandle = (type) => {
    ...
    nextTick(() => {
        setTimeout(() => {
            emits('callabck', getSelectedValue(rightTreeRef.value.root.childNodes, []))
        }, 1000)
    })
};
//获取右侧穿梭框所有叶子节点值
const getSelectedValue = (nodes, keys) => {
    nodes.forEach(i => {
        if (i.isLeaf && i.visible) {
            keys.push(i.data[props.keyCode])
        }
        if (i.childNodes.length && i.visible) {
            getSelectedValue(i.childNodes, keys)
        }
    })
    return keys
}
...
const setSelectNodes = (nodes, keys, type) => {
    console.log('setSelectNodes', nodes, keys);
