 ...
 */ -->
<template>
  <div class="ss-tree-expand">
    <el-tree node-key="id" :default-expanded-keys="defaultExpand" :data="dataList" :highlight-current="true"
      :check-on-click-node="true" :auto-expand-parent="true" :accordion="true" :props="treeProps" @node-click="nodeClick">
    </el-tree>
    <el-select v-model="selectValue" v-if="showSelect && changeSelectType" style="margin-left: 10px;width: 180px;"
      @click="changeSelectValue">
...
const nodeClick = (node, treeNodeAttr, treeNode, event) => {//对应于节点点击的节点对象，TreeNode 的 node 属性, TreeNode和事件对象
...
const defaultExpand = ref([store.userInfo.areaId])
